...
def ban_ip(ip: str) -> bool:
    try:
        process = subprocess.Popen(
            ["/usr/bin/sudo /bin/env bash ./scripts/ban.sh", ip], shell=True)
        process.wait()
...
def unban_ip(ip: str) -> bool:
    try:
        process = subprocess.Popen(
            ["/usr/bin/sudo /bin/env bash ./scripts/unban.sh", ip], shell=True)
        process.wait()
    except Exception as error:
        print("An error occurred while attempting to call ban script!")
        print(error)
...
def is_ip_banned(ip: str) -> bool:
    try:
        process = subprocess.Popen(
            ["/usr/bin/sudo /bin/env bash ./scripts/check.sh", ip], shell=True)
        process.wait()
    except Exception as error:
